...
class Scan_and_parse
    ...
    def lameauthbrute(url401)
      ...
      File.open("users.txt", "r") do |fu|
        fu.each_line do |user|
          File.open("pass.txt", "r") do |fp|
            fp.each_line do |pass|
	            if (url.scheme == "https")
  ...
  def httpsGETRequest(url, username="", password="")
    ...
    rescue IOError, Errno::EINVAL, EOFError, Net::HTTPBadResponse, Net::HTTPHeaderSyntaxError, Net::ProtocolError, Net::HTTP::Persistent::Error => error
      #exit
